...
本平台通过整合底层大模型能力、RAG（检索增强生成）知识库以及 AI 安全防护机制，向上对接互联网及内网用户，中台实现智能体的全生命周期管理（AgentMgt、UserMgt、RAGMgt），核心层通过"任务与领域（Task & Domain）"引擎实现业务逻辑的自动化编排，并通过强大的连接器（Connection）触达 ERP、数据库、WeChat 等各类生产系统。`,
    ...
    values: [
      { title: "降低开发门槛", desc: "业务人员可通过配置 AgentMgt 和 Task 模块快速���线智能体，无需深度编写 LLM 调用代码。" },
...
结果：Agent 生成数据驱动的分析结论，支撑决策。` },
  ...
  },
  {
    ...
    heroImage: "/images/solutions/solutions-banner.png",
    overview: `在数字化转型与信创改造的双重驱动下，核心业务系统对中间件的高可用性提出了更高要求。传统主备容灾模式存在切换中断、数据丢失、RTO/RPO难以保障等痛点，已无法满足金融、电信、能源等关键行业的连续性要求。中创信创中间件双活容灾解决方案，基于自主研发的数据同步工具软件，提供准实时数据同步能力，实现消息中间件、缓存中间件的本地或异地集群间数据热备份。方案支持数据双向同步，两侧集群均处于活跃可用状态，任意一侧均可独立承载业务读写，真正实现"双活"架构。`,
    overviewHighlight: "提供热备能力，服务随时可用，业务应用容灾切换的工作量小、操作少、耗时短。",
    highlights: [
      { icon: "deploy", title: "双活保畅", desc: `区别于传统的"冷备"模式，本方案实现真正的双活架构。两侧集群均可同时提供读写服务，无需等待数据同步完成，无需���时启动服务。当故障发生时，业务流量可直接切换至健康集群，整个过程近乎无感知，RTO趋近于零，保障业务连续性。` },
  ...
  },
  {
    ...
    features: [
      ...
      { title: "实时数据流分��", desc: "基于实时数据流构建多维度数据模型，为业务场景提供实时分析能力。" },
  ...
  },
  {
    ...
    features: [
      ...
      { title: "动态拓扑与关联分析", desc: "自动发现并生成反映组件间依赖关系的应用与网络拓扑，结合实时性能数据，实现\"一张图\"式可视、可管的关联运维。" },
