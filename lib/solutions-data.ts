...
export const solutions: SolutionDetail[] = [
  {
    ...
    overview: "随着大语言模型（LLM）技术的爆发，企业对 AI 的需求已从简单的对话问答转向深度的业务协同。本解决方案旨在构建一套以任务为导向、以领域为核心的企业级智能体管理平台。本平台通过整合底层大模型能力、RAG（检索增强生成）知识库以及 AI 安全防护机制，向上对接互联网及内网用户，中台实现智能体的全生命周期管理（AgentMgt、UserMgt、RAGMgt），核心层通过任务与领域（Task & Domain）引擎实现业务逻辑的自动化编排，并通过强大的连接器（Connection）触达 ERP、数据库、WeChat 等各类生产系统。",
    highlights: [
