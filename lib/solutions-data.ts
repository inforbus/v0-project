...
export const solutions: SolutionDetail[] = [
  ...
  },
  {
    ...
    useCases: [
      ...
      { title: "金融行业大数据业务化转型", desc: "帮助金融机构构建大数据平台，实现客户、产品、交易等数据的统��治理，�����撑精准营销和风控决策。" },
...
结果：Agent 生成数据驱动的分析结论，支撑决策。` },
  ...
  },
  {
    ...
    highlights: [
      ...
      { icon: "govern", title: "优化资源投资，实现精细管控", desc: "清晰的资源账本与性能容量分析，帮助识别资��瓶颈与浪费，为IT投资与扩容决策提供精准数据依据，降低成本。" },
    ...
    features: [
      ...
      { title: "智能告警与根因诊断", desc: "支持灵活的告警规则与丰富通知方式。通过告警压缩、关联分析与AI根���定位，快速收敛问题，直达故障本源。" },
...
中创信创中间件双活容灾解决方案，基于自主研发的数据同步工具软件，提供准实时数据同步能力，实现消息中间件、缓存中间件的本地或异地集群间数据热备份。方案支持数据双向同步，两侧集群均处于活跃可用状态，任意一侧均可独立承载业务读写，真正实现"双活"架构。`,
    ...
    highlights: [
      ...
      { icon: "monitor", title: "高性能", desc: "采用增量数据捕获与高效传输协议，同步效率达到业界领先水平。数据同步延迟低至毫秒级，确保两侧集群数据高度一致，满足��频交易、实时发布等对时效性要求高的业务场景。" },
    ...
    features: [
      ...
      { title: "进度监控", desc: "提供完善的同步状态监控能力，采集同步延迟、吞吐量、堆积量等关键指标，运维人员可��面掌握同步链路健康状态。" },
    ],
    benefits: [
