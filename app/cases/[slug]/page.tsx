...
> = {
  "central-enterprise-middleware-platform": {
    background:
      "某央企作为国家支柱产业的重要组成，需要构建全栈式自主可控的中间件统一管理平台。该平台需要支撑集团内部海量数据处理、复杂业务流程管理，并确保系统的高可用性和安全可控性。集团要求平台能够统一管理消息中间件、应用服务器、缓存引擎等核心组件，实现从应用层到基础设施层的全链路监控与管理。",
    challenge: [
      "日均数据集超百亿10亿+，需要高效存储和处理能力",
      "多区域数据流转需求，保证数据完整性和一致性",
      "7x24小时不间断服务，需要达成99.99%以上的可用性",
      "需要自主可控的技术体系，满足信创战略要求",
    ],
    solution: [
      "采用自主可控的InforSuite系列产品进行系统建设，实现全栈中间件国产化",
      "构建高可用架构，实现跨数据中心容灾备份和多活部署",
      "建立完善的监控预警体系，确保系统稳定运行",
      "提供端到端的安全防护和合规管理方案",
    ],
    results: [
    ...
    contentSections: [
      {
        title: "项目背景",
        text: "某央企作为国家支柱产业的重要组成，需要构建全栈式自主可控的中间件统一管理平台。该平台需要支撑集团内部海量数据处理、复杂业务流程管理，并确保系统的高可用性和安全可控性。",
        image: "/images/cases/central-background.png",
      },
      {
        title: "项目成果",
        text: "平台成功部署后，实现了对集团各类中间件的统一管理。支撑日均数据处理6亿+条，日均调度次数10亿+次，平均响应时间<50ms，系统可用性达到99.99%。在跨地域、跨数据中心的复杂业务场景下，平台充分发挥了自主可控中间件的优势。",
        image: "/images/cases/central-achievement.png",
      },
      {
        title: "项目架构",
  ...
  "intelligent-ops-platform": {
    background:
      "某大型能源集团在全国运营着数百套业务系统，中间件运维主要依赖人工巡检和被动响应，运维效率低、成本高。为实现降���增效，需要建设一套智能化的中间件运维管理平台，实现从被动运维到主动运维的转变。",
    challenge: [
      "系统分布广泛，跨多个数据中心和地域",
      "中间件类型多样，版本众多，管理复杂",
      "运维人员技能水平参差不齐，故障响应慢",
